...
export function ServiceDialog({ isOpen, setIsOpen }: props) {
  ...
  const onSubmit = async () => {
    try {
      ...
      const err =
        error.response.message || error.message || "Fail to submit service";
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-175 rounded-none border-4 border-slate-950 p-0 bg-white shadow-[30px_30px_0px_0px_rgba(180,130,20,0.2)]">
        {/* --- DIALOG HEADER --- */}
        <div className="p-10 bg-slate-50 border-b-4 border-slate-950 relative">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-[10px] uppercase tracking-[0.5em] text-amber-700 font-black">
              Add new Service Attract Your User{" "}
            </h2>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="p-10 space-y-10 max-h-[60vh] overflow-y-auto">
            ...
            <div className="space-y-5">
              ...
              <Tabs defaultValue="en" className="w-full ">
                <TabsList className="grid w-full cursor-pointer grid-cols-3 rounded-none bg-gray-500 border-2 border-slate-950 p-1 h-14">
                  <TabsTrigger
                    value="en"
          ...
          <div className="p-10 bg-slate-50 border-t-4 border-slate-950 flex flex-row justify-end gap-6">
            ...
            <Button
              className="rounded-none bg-slate-950 text-white hover:bg-amber-700 h-16 px-12 font-black uppercase text-xs tracking-[0.2em] shadow-[8px_8px_0px_0px_rgba(180,130,20,0.3)] cursor-pointer"
              type="submit"
            >
              <Plus size={18} className="mr-2" />
              {isSubmitting ? "submiting" : "Save Service"}
            </Button>
          </div>
        </form>
      </DialogContent>
